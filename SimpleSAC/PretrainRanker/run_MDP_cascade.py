...
def run(train_set, test_set, ranker, eta, reward_method, num_interation,
    ...
    for i in index:
        ...
        clicked_doces, click_labels, obs_probs, _ = click_model.simulate_cascade(
            query = qid,
...
def job(model_type, learning_rate, eta, reward_method, f, train_set, test_set,
    ...
    if model_type == "perfect":
        pc = [0.0, 0.5, 1.0]
        ps = [0.0, 0.0, 0.0]  # always go through the whole list
    elif model_type == "navigational":
        pc = [0.05, 0.5, 0.95]
        ps = [0.2, 0.5, 0.9]
    elif model_type == "informational":
    ...
    if not os.path.exists("./results_MDP_cascade/Fold{}/{}".format(f,model_type)):
        os.makedirs("./results_MDP_cascade/Fold{}/{}".format(f,model_type))
    if not os.path.exists(f"./results_MDP_cascade/Fold{f}/{model_type}/checkpoints"):
        os.makedirs(f"./results_MDP_cascade/Fold{f}/{model_type}/checkpoints")
    ...
    for r in range(1, 3):
        ...
        # SummaryWriter
        writer = SummaryWriter('./results_MDP_cascade/Fold{}/{}'.format(f,model_type))
        ...
        # model save
        checkpoint_path = f"./results_MDP_cascade/Fold{f}/{model_type}/checkpoints"
