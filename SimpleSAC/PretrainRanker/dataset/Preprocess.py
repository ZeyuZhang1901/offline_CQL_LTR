...
class PreprocessDataset(AbstractDataset):
    ...
    def from_path(self, path: str, cache_root: str) -> 'PreprocessDataset':
        """
        Constructs a dataset by reading from a disk
    ...
    def _load_data(self):
        ...
        data format:
            ...
            A query-document pair is represented by a 46-dimensional feature vector. Here are several example rows from MQ2007 dataset:
        =================================
        ...
        with open(self._path, "r") as fin:
            ...
            for line in fin:  
                ...
                if query == current_query:  # Old queries
                ...
                # relevance parts
                relevence = float(
                    cols[0])  # Sometimes the relevance label can be a float.
                if relevence.is_integer():
                    relevence = int(
                        relevence
                    )  # But if it is indeed an int, cast it into one.
                if self._binary_label != 0:  # if use binary relevance
                ...
                for i in range(2, len(cols)):
                    ...
                    if not feature_id.isdigit():  # last col
                        if feature_id[0] == "#":  # match docid to query
                    ...
                    if math.isnan(feature_value):  # deal with Nan value
                        feature_value = 0
                ...
                if old_query:
                    self._query_docid_get_features[query][docid] = np.array(
                        features)
                    self._query_get_docids[query].append(docid)
                    self._query_get_all_features[query] = np.vstack(
                    ...
                    self._query_docid_get_features[query] = {
                        docid: np.array(features)
                    }
    ...
    def _normalise(self, query):  # linear normalization
        norm = np.zeros(
            (len(self._query_get_docids[query]), self._feature_size))
        ...
        if norm.shape[0] != 1:
            ...
            safe_ind = max - min != 0
            norm[:,
                 safe_ind] = (query_features[:, safe_ind] -
                              min[safe_ind]) / (max[safe_ind] - min[safe_ind])
    ...
    def write(self, output_file):
        ...
        for query in self.get_all_querys():
            ...
            for i, features in enumerate(
                    self.get_all_features_by_query(query)):
                comment = comments[i]
                ...
                s += "{} qid:{} {}#{}\n".format(label, query, features_str,
                                                comment)
